/**
 * @citta/integrations — PMPay Payment Integration Service (CONDIVISO office + portal)
 * Client HTTP puro per PagoPA via PmPay cloud. Nessuna dipendenza app-locale
 * (no prisma, no @/): tutto via env + fetch. L'orchestrazione DB (es. sync cron)
 * resta lato app e usa questo client.
 *
...
class PMPayService {
  ...
  /** Scarica il PDF della ricevuta telematica per il dato IUV (usato dal portale). */
  async getRicevuta(iuv: string): Promise<GetBollettinoResponse> {
    if (!this.isConfigured()) {
      return { success: false, error: 'PMPay non configurato' };
    }
    try {
  ...
  // NB: syncPendingPayments (cron) resta lato office in src/lib/external/pmpay.ts
  // perché dipende dal client Prisma app-locale. Usa questo pmPayService.getPaymentDetail.
}
