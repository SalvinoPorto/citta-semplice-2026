...
import { getCampoValue } from '@citta/form-schema';
import {
  costruisciRiepilogo,
  parseCampi as parseSchema,
  type VoceRiepilogo,
} from '@citta/form-schema';
import { getStorage } from '@citta/storage';
...
export type DatiIstanzaDoc = {
  ...
  /**
   * `true` quando `protoNumero` è ancora il numero INTERNO d'emergenza.
   *
   * Obbligatorio di proposito, senza valore di default: questo documento
   * dichiara di essere stato protocollato dal Protocollo generale dell'ente, e
   * un chiamante che dimenticasse il campo produrrebbe quell'affermazione su una
   * ricevuta dove non è (ancora) vera. Meglio un errore di compilazione.
   */
...
async function salvaPdf(buffer: Buffer): Promise<string> {
  ...
  await getStorage().save(nomeHash, buffer, 'application/pdf');
  return nomeHash;
}
        ...
        {istanza.protoNumero ? (
          <View style={s.protoBox} wrap={false}>
            <View style={s.protoHead}>
              <Text style={s.protoHeadText}>
                {istanza.protocolloProvvisorio ? 'Protocollo provvisorio' : 'Protocollo'}
              </Text>
            </View>
    ...
    <Page size="A4" style={s.page}>
      ...
      <View style={s.ricIntro} wrap={false}>
        {istanza.protocolloProvvisorio ? (
          // Il numero è ancora quello interno: affermare che l'istanza è stata
          // protocollata dal Protocollo generale sarebbe falso su un documento
          // con valore amministrativo. Si dichiara ciò che è vero adesso.
          <Text>
            L&apos;istanza/segnalazione/comunicazione è stata acquisita dal{' '}
            <Text style={s.strong}>{nomeEnte}</Text> con numero provvisorio{' '}
            <Text style={s.strong}>{istanza.protoNumero ?? '—'}</Text> del{' '}
            <Text style={s.strong}>{formatData(istanza.protoData)}</Text>. La registrazione
            al Protocollo generale è in corso: il numero definitivo sarà
            consultabile nell&apos;area personale del portale.
          </Text>
        ) : (
          <Text>
